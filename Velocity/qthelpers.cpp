...
void QtHelpers::SearchTreeWidget(QTreeWidget *widget, QLineEdit *searchWidget, QString searchString)
{
    ...
    {
        QtHelpers::SetErrorStyle(searchWidget);
        for (int i = 0; i < widget->topLevelItemCount(); i++)
        {
    ...
    QtHelpers::ClearErrorStyle(searchWidget);
...
void QtHelpers::SetErrorStyle(QWidget *widget)
{
    // Use theme-aware error styling
    // Instead of hardcoded red, use a stylesheet that works in both light and dark modes
    QPalette palette = widget->palette();
    bool isDark = (palette.color(QPalette::Window).lightness() < 128);
    
    if (isDark) {
        // Dark mode: use light red
        widget->setStyleSheet("color: rgb(255, 120, 120);");
    } else {
        // Light mode: use darker red for better contrast
        widget->setStyleSheet("color: rgb(200, 0, 0);");
    }
}

void QtHelpers::ClearErrorStyle(QWidget *widget)
{
    // Clear the stylesheet to revert to theme colors
