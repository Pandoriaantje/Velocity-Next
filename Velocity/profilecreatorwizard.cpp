...
void ProfileCreatorWizard::onFinished(int status)
{
    ...
    newProfile.metaData->thumbnailImage.assign(thumbStart, thumbStart + ba1.size());
    ...
    newProfile.metaData->titleThumbnailImage.assign(titleThumbStart, titleThumbStart + ba2.size());
...
void ProfileCreatorWizard::on_txtGamertag_textChanged(const QString & /* arg1 */)
{
    ...
    {
        QtHelpers::SetErrorStyle(ui->txtGamertag);
        button(QWizard::NextButton)->setEnabled(false);
    }
    else
    {
        QtHelpers::ClearErrorStyle(ui->txtGamertag);
